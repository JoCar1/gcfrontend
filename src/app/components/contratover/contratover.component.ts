...
import { Icontrato, Icuota, Irecordatorio, Iorden, Iresponsable, Iuser , Iproveedor} from 'src/app/models/interfaces';
...
@Component({
  selector: 'app-contratover',
  templateUrl: './contratover.component.html',
  styleUrls: ['./contratover.component.scss'],
...
export class ContratoverComponent implements OnInit {
  ...
  @ViewChild('autoprov', { static: false }) matAutocompleteprov: MatAutocomplete;
...
this.proveedoresAll().then(() => {
  ...
  //   })
    ...
    this.route.paramMap.subscribe(params => {
      if (params.get('codigo_oc')!=null) {
        this.codigo_oc = params.get('codigo_oc').toString().replace('_', '/');      
      }    
...
this.restangular.one('cuotas').get({ codigo_oc: this.codigo_oc}).subscribe(response => {
    ...
    if (this.codigo_oc!=null) {
      ...
      this.restangular.one('ordenver').get({ codigo: this.codigo_oc }).subscribe(response => {
        ...
        console.log('se descargo la orden;: '+this.orden);
      ...
      this.restangularContrato.one('contratover').get({ codigo: this.codigo_oc,  sistema:this.sistema }).subscribe(response => {
        ...
        if (this.contratoid === 0 || this.contratoid == undefined) {
          ...
          this.contrato.numero = this.orden.codigo;
        }
        else {
  ...
  proveedoresAll() {
    return new Promise(resolve => {
      this.restangular.one('proveedorestall').get('').subscribe(
        (data) => {
  ...
  guardarOrdenC() {
    ...
    let data = this.dataFormOrden.value;
    let datos = new FormData();  
    datos.append('codigo',data.codigo);
    datos.append('descr_tipo',data.descr_tipo);
    datos.append('provedor',this.proveedorSeleccionado.codigo);
    datos.append('nombre_proveedor', this.proveedorSeleccionado.nombre);
    datos.append('sistema_p', this.proveedorSeleccionado.sistema);

    // let data = this.dataFormContrato.getRawValue();
    this.spinner.show();
    console.log('PROVEEDOR AGREGADO:'+datos);
    this.restangular.all('ordenc').post(datos).subscribe(
      (response) => {
        this.codigo_oc=data.codigo;
        this.ngOnInit();
  ...
  actualizarOrdenC() {
    ...
    console.log(data);
    this.spinner.show();
    this.restangular.one('ordenc', this.contratoid).put(data).subscribe(
      (response) => {
        console.log('Guardado, agregado');
        console.log(response);
        this.spinner.hide();
  ...
  guardarContrato() {
    ...
    this.restangular.all('contratos').post(data).subscribe(
      (response) => {
        // this.codigo_oc=this.orden.codigo;
        this.ngOnInit();
        console.log('Guardado, agregado');
        console.log(response);
  ...
  filterProveedores(value: string | Iproveedor): Iproveedor[] {
    const filterValue = typeof value === 'string' ? value.toLowerCase() : '';
    return this.proveedores.filter(proveedor => proveedor.nombre.toLowerCase().includes(filterValue));
  }
  
  selectedprov(event: MatAutocompleteSelectedEvent): void {
    this.proveedorCtrl.setValue(null);
    this.proveedorSeleccionado = event.option.value as Iproveedor;
    console.log(this.proveedorSeleccionado);
  }
  
  getProveedorNombre(): string {
    return this.proveedorSeleccionado ? this.proveedorSeleccionado.nombre : '';
  }
  
  getProveedorNit(): string {
    return this.proveedorSeleccionado ? this.proveedorSeleccionado.nit : '';
  }
